...
local Planner = mc.class("Planner", IContext)
...
Planner.OnNewPlan = function() end
...
Planner.OnCurrentTaskExecutingConditionFailed = function() end
...
function Planner:Tick(domain, ctx, allowImmediateReplan)
    ...
    if (self._currentTask == {} and (#self._plan == 0) or ctx.IsDirty) then
        ...
        if (decompositionStatus == EDecompositionStatus.Succeeded or decompositionStatus == EDecompositionStatus.Partial
            ...
            if (self._currentTask ~= {} and self._currentTask:isSubclassOf(IPrimitiveTask)) then
    ...
    if (self._currentTask ~= {}) then
        if (self._currentTask:isSubclassOf(IPrimitiveTask)) then

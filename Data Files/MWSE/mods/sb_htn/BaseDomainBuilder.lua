...
local ICompoundTask = require("Tasks.CompoundTasks.ICompoundTask")
local IPrimitiveTask = require("Tasks.PrimitiveTasks.IPrimitiveTask")
local PausePlanTask = require("Tasks.CompoundTasks.PausePlanTask")
local FuncCondition = require("Conditions.FuncCondition")
local FuncOperator = require("Operators.FuncOperator")
local ActionEffect = require("Effects.ActionEffect")
local Slot = require("Tasks.OtherTasks.Slot")
...
function BaseDomainBuilder:CompoundTask(name, task)
    ...
    assert(self:Pointer().Subtasks ~= nil,
...
function BaseDomainBuilder:PausePlanTask()
    ...
    self._domain.AddTask(compoundTask, parent);
...
function BaseDomainBuilder:ExecutingCondition(name, condition)
    assert(self:Pointer().ExecutingConditions ~= nil,
...
function BaseDomainBuilder:Effect(name, effectType, action)
    assert(self:Pointer().ExecutingConditions ~= nil, "Tried to add an Effect, but the Pointer is not a Primitive Task!")
...
function BaseDomainBuilder:Splice(domain)
    assert(self:Pointer().Subtasks ~= nil, "Pointer is not a compound task type. Did you forget an End()?")
